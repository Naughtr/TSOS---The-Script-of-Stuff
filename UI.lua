...
-- Safely grab CoreGui to prevent thread-halting security errors
local CoreGui
...
return function(plr, CFG, callbacks)
    ...
    -- Safe Parent Target Resolution
    local targetParent = (gethui and gethui()) or CoreGui or plr:WaitForChild("PlayerGui")
    local oldGui = targetParent:FindFirstChild("ToolsGUI")
    if oldGui then oldGui:Destroy() end
    
    gui = mk("ScreenGui", targetParent, {Name="ToolsGUI", ResetOnSpawn=false, IgnoreGuiInset=true, DisplayOrder=9e8})
    ...
    function UI.createSpeedometerUI(existingPos)
        ...
        UI.spdGui = mk("ScreenGui", targetParent, {Name="TSOS_Speedometer", ResetOnSpawn=false, IgnoreGuiInset=true, DisplayOrder=100000, ZIndexBehavior=Enum.ZIndexBehavior.Global})
    ...
    function UI.createJitterUpUI(activateCallback)
        ...
        UI.jfUpGui = mk("ScreenGui", targetParent, {Name="TSOS_JitterUp", ResetOnSpawn=false, IgnoreGuiInset=true, DisplayOrder=999999999, ZIndexBehavior=Enum.ZIndexBehavior.Global})
    ...
    function UI.loadRayfield(onSuccess, onFail, currentState, toggleFuncs)
        ...
        SettingsTab:CreateButton({
            ...
            Callback = function()
                ...
                local RayfieldGui = (gethui and gethui():FindFirstChild("Rayfield")) or (CoreGui and CoreGui:FindFirstChild("Rayfield"))
    ...
    UI.API = {
        playAnim = function()
            ...
            -- Adjusted the yield from 4 seconds down to 1.5 for a much faster boot up
            local s=mk("TextLabel", gui, {Size=ud2(1,0,1,0), BackgroundTransparency=1, Text="TSOS", TextColor3=c3(255,255,255), Font=Enum.Font.GothamBold, TextSize=100, ZIndex=100}); task.wait(1.5); tw(s, tFast, {TextTransparency=1}, true); s:Destroy()
