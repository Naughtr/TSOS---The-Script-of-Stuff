...
-- New: Snapping tween for smooth alignment
...
return function(plr, CFG)
    ...
    local function stBAct(b, act) 
        if act then updBClr(b, Color3.fromHSV(math.random(), 0.75, 0.45), true) 
        elseif bOrigClr[b] then updBClr(b, bOrigClr[b], false) end 
    end
    ...
    local function rndBClr() 
        ...
        for _, b in ipairs(btns) do 
            ...
            local c = Color3.fromHSV(h, 0.7, 0.4); bOrigClr[b] = c; updBClr(b, c, false) 
        end 
    end
    ...
    cScrl = mk("ScrollingFrame", main, {Name="ConfigFrame", Size=ud2(1,-16,0,52), Position=ud2(0,8,0,36), BackgroundColor3=CFG.BACKGROUND_COLOR, ScrollBarThickness=2, CanvasSize=ud2(0,0,0,0), Visible=false, ScrollingDirection=Enum.ScrollingDirection.Y, ElasticBehavior=Enum.ElasticBehavior.Always})
    ...
    local function handleSnap(f)
        ...
        local targetY = math.clamp(math.round((currentY - topOff) / step) * step + topOff, 0, math.max(0, f.CanvasSize.Y.Offset - f.AbsoluteSize.Y))
    ...
    for _, f in ipairs({scrl, cScrl}) do
        f:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
            ...
            local t = tick(); lastScrollTimes[f] = t
    ...
    local function sB(nm, tx)
        ...
        mk("UICorner", bg, {CornerRadius=UDim.new(0,4)}); mk("UIPadding", bg, {PaddingLeft=UDim.new(0,0)}); mk("UIGradient", bg, {Rotation=0}); local st=mk("UIStroke", bg, {Thickness=1, ApplyStrokeMode=Enum.ApplyStrokeMode.Border, Color=c3(255,255,255), Transparency=1})
    ...
    local function unTrn(t) tw(t, tBncIn, {Size=ud2(0,0,0,0), Position=ud2(0.5,0,0.5,0)}, true); t.Visible=false; local cx=main.Position.X; main.Size, main.Position, main.Visible = ud2(0,0,0,22), ud2(cx.Scale,cx.Offset,main.Position.Y.Scale,main.Position.Y.Offset), true; tw(main, tSmth, {Size=ud2(0,120,0,22), Position=ud2(cx.Scale, cx.Offset-60, main.Position.Y.Scale, main.Position.Y.Offset)}, true); fdMnu(0, true); shwUi(true, 120, 126); setA(0) end
    ...
    function UI_API.playAnim()
        ...
        main.Visible, main.Size, main.Position = true, ud2(0,0,0,22), ud2(0.5,0,0.5,-59); tw(main, tSmth, {Size=ud2(0,120,0,22), Position=ud2(0.5,-60,0.5,-59)}, true); fdMnu(0, true); shwUi(true, 120, 126); tgBtns(0, 0.05); setA(0)
    end

    function UI_API.toggleConfigMenu(isOpen)
        if isAnimating then return end
        tw(logo, tBnc, {Rotation=logo.Rotation+360}); setA(1); task.wait(0.1); isAnimating = true 
        if not isOpen then tgCfg(1); task.wait(0.15); tw(main, tSmth, {Size=ud2(0,120,0,22)}, true); cScrl.Visible, scrl.Visible = false, true; shwUi(true, 120, 126); tgBtns(0)
        else tgBtns(1); task.wait(0.15); tw(main, tSmth, {Size=ud2(0,120,0,22)}, true); scrl.Visible, cScrl.Visible = false, true; shwUi(true, 120, 126); tgCfg(0) end; setA(0); isAnimating = false
    end
    ...
    function UI_API.drawPlayerEsp(p, c, tp, espState, dist, clr, sp, isOffscreen, vpCtr, bx, by)
        if espState>=1 and espState<=4 then
            if not espHL[p] or espHL[p].Parent~=c then if espHL[p] then espHL[p]:Destroy() end; espHL[p] = mk("Highlight", c, {Name="EspChams", FillTransparency=0.7, OutlineTransparency=0.2, OutlineColor=c3(255,255,255), FillColor=clr})
            else espHL[p].FillColor=clr end
        elseif espHL[p] then espHL[p]:Destroy(); espHL[p]=nil end
        if espState>=1 and espState<=3 then
            local txl=p.Name.."\n["..dist.." studs]"
            if not espTg[p] or espTg[p].Parent~=tp then if espTg[p] then espTg[p]:Destroy() end; local b=mk("BillboardGui", tp, {Name="EspNametag", AlwaysOnTop=true, Size=ud2(0,120,0,30), MaxDistance=math.huge, Adornee=tp}); mk("TextLabel", b, {Name="Label", Size=ud2(1,0,1,0), BackgroundTransparency=1, Text=txl, TextColor3=clr, TextStrokeTransparency=0, TextStrokeColor3=c3(0,0,0), Font=Enum.Font.GothamBold, TextSize=10}); espTg[p]=b
        ...
        if isOffscreen then
            ...
            local l, off = espOff[p], (Vector2.new(sp.X, sp.Y) - vpCtr); if sp.Z < 0 then off = -off end; local dir = off.Magnitude < 0.001 and Vector2.new(0, 1) or off.Unit; local scale = math.min(bx / math.abs(dir.X), by / math.abs(dir.Y)); local pos = vpCtr + (dir * scale); l.Text, l.TextColor3, l.Position, l.Visible = p.Name.."\n["..dist.." studs]", clr, ud2(0, pos.X, 0, pos.Y), true
