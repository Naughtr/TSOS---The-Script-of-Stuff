...
return function(plr, CFG)
    ...
    local function updBClr(b, c, act)
        ...
        if gr then gr.Enabled = not act; gr.Color = ColorSequence.new(c, c3(15,15,15)) end
        ...
        if st then 
            ...
            if sg then sg.Enabled = not act; local h,s,v = c:ToHSV(); sg.Color = ColorSequence.new(Color3.fromHSV(h, s*0.8, math.min(v*1.4, 1)), c3(0,0,0)) end
        end
    end
    ...
    tTabBg = mk("Frame", main, {Name="TitleTabBg", Size=ud2(1,0,0,22), BackgroundColor3=CFG.TAB_COLOR, BorderSizePixel=0}); mk("UICorner", tTabBg, {CornerRadius=UDim.new(0,10)}); mk("Frame", tTabBg, {Name="BottomBorder", Size=ud2(1,0,0,1), Position=ud2(0,0,1,-1), BackgroundColor3=CFG.BORDER_COLOR, BorderSizePixel=0, ZIndex=3})
    ...
    -- Scrolling Viewports
    scrl = mk("ScrollingFrame", main, {Size=ud2(1,-16,0,52), Position=ud2(0,8,0,32), BackgroundColor3=CFG.BACKGROUND_COLOR, ScrollBarThickness=2, CanvasSize=ud2(0,0,0,0), ScrollingDirection=Enum.ScrollingDirection.Y, ElasticBehavior=Enum.ElasticBehavior.Always})
    ...
    uiLL:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() scrl.CanvasSize = ud2(0, 0, 0, uiLL.AbsoluteContentSize.Y + uiPad.PaddingTop.Offset + uiPad.PaddingBottom.Offset) end)

    cScrl = mk("ScrollingFrame", main, {Name="ConfigFrame", Size=ud2(1,-16,0,52), Position=ud2(0,8,0,32), BackgroundColor3=CFG.BACKGROUND_COLOR, ScrollBarThickness=2, CanvasSize=ud2(0,0,0,0), Visible=false, ScrollingDirection=Enum.ScrollingDirection.Y, ElasticBehavior=Enum.ElasticBehavior.Always})
    local cfPad = mk("UIPadding", cScrl, {PaddingTop=UDim.new(0,4), PaddingBottom=UDim.new(0,4), PaddingLeft=UDim.new(0,2), PaddingRight=UDim.new(0,2)})
    local cfLL=mk("UIListLayout", cScrl, {Padding=UDim.new(0,4), HorizontalAlignment=Enum.HorizontalAlignment.Center})
    cfLL:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() cScrl.CanvasSize=ud2(0,0,0,cfLL.AbsoluteContentSize.Y + cfPad.PaddingTop.Offset + cfPad.PaddingBottom.Offset) end)
    ...
    for _, k in ipairs(sk) do 
        local r=mk("Frame", cScrl, {Size=ud2(0.9,0,0,20), BackgroundTransparency=1}); 
        local l=mk("TextLabel", r, {Size=ud2(0.5,0,1,0), BackgroundTransparency=1, Text=sNm[k] or k, TextColor3=CFG.TEXT_COLOR, TextXAlignment=Enum.TextXAlignment.Center, Font=Enum.Font.Gotham, TextSize=7, TextTransparency=1, Active=true})
    ...
    -- Snapping Logic
    local lastScrollTimes, snapDebounce = {}, {}
    local function handleSnap(f)
        if snapDebounce[f] then return end; snapDebounce[f] = true
        local step, topOff = 24, (f:FindFirstChildOfClass("UIPadding") and f:FindFirstChildOfClass("UIPadding").PaddingTop.Offset or 0)
        local curY = f.CanvasPosition.Y
        local targetY = math.clamp(math.round((curY - topOff) / step) * step + topOff, 0, math.max(0, f.CanvasSize.Y.Offset - f.AbsoluteSize.Y))
        if math.abs(curY - targetY) > 0.5 then local t = TS:Create(f, snapInfo, {CanvasPosition = Vector2.new(0, targetY)}); t:Play(); t.Completed:Wait() end
        snapDebounce[f] = false
    end
    for _, f in ipairs({scrl, cScrl}) do f:GetPropertyChangedSignal("CanvasPosition"):Connect(function() local t = tick(); lastScrollTimes[f] = t; task.delay(0.15, function() if lastScrollTimes[f] == t then handleSnap(f) end end) end) end
    ...
    local function sB(nm, tx)
        ...
        mk("UICorner", bg, {CornerRadius=UDim.new(0,4)}); mk("UIStroke", bg, {Thickness=1, ApplyStrokeMode=Enum.ApplyStrokeMode.Border, Color=c3(255,255,255), Transparency=1})
        b.MouseButton1Down:Connect(function() if isAnimating then return end tw(b, tFast, {Size=ud2(0.85,0,0,18)}) end)
        local u=function() if isAnimating then return end tw(b, tBnc, {Size=ud2(0.9,0,0,20)}) end; b.MouseButton1Up:Connect(u); b.MouseLeave:Connect(u); table.insert(btns, b); return b
    end
    ...
    local function cnfEx(v) tw(cnfLbl,tFast,{TextTransparency=v}); tw(bYes,tFast,{TextTransparency=v}); tw(bNo,tFast,{TextTransparency=v}); tw(bYes.Background,tFast,{BackgroundTransparency=v}); tw(bNo.Background,tFast,{BackgroundTransparency=v}); tw(bYes.Background.UIStroke,tFast,{Transparency=v}); tw(bNo.Background.UIStroke,tFast,{Transparency=v}) end
    local function inEx(v) tw(inBox,tFast,{TextTransparency=v}); tw(bSrch,tFast,{TextTransparency=v}); tw(bCnc,tFast,{TextTransparency=v}); tw(bSrch.Background,tFast,{BackgroundTransparency=v}); tw(bCnc.Background,tFast,{BackgroundTransparency=v}); tw(bSrch.Background.UIStroke,tFast,{Transparency=v}); tw(bCnc.Background.UIStroke,tFast,{Transparency=v}) end
    ...
    function UI_API.setLagVisual(hrpCFrame, isActive)
        ...
        elseif lagPrt then lagPrt:Destroy() lagPrt=nil end
    end
    ...
    function UI_API.setWarningGui(hrp, isVisible)
        ...
        wrnGui.Parent, wrnGui.Adornee = hrp, hrp; wrnGui.Enabled = isVisible
    end
    ...
    function UI_API.playAnim()
        ...
        main.Visible, main.Size, main.Position = true, ud2(0,0,0,22), ud2(0.5,0,0.5,-59); tw(main, tSmth, {Size=ud2(0,120,0,22), Position=ud2(0.5,-60,0.5,-59)}, true); fdMnu(0, true); shwUi(true, 120, 124); tgBtns(0, 0.05); setA(0)
    end

    function UI_API.toggleConfigMenu(isOpen)
        if isAnimating then return end; tw(logo, tBnc, {Rotation=logo.Rotation+360}); setA(1); task.wait(0.1); isAnimating = true 
        if not isOpen then tgCfg(1); task.wait(0.15); tw(main, tSmth, {Size=ud2(0,120,0,22)}, true); cScrl.Visible, scrl.Visible = false, true; shwUi(true, 120, 124); tgBtns(0)
        else tgBtns(1); task.wait(0.15); tw(main, tSmth, {Size=ud2(0,120,0,22)}, true); scrl.Visible, cScrl.Visible = false, true; shwUi(true, 120, 124); tgCfg(0) end; setA(0); isAnimating = false
    end

    function UI_API.minimize() if isAnimating then return end; bMin.Visible, bCls.Visible = false, false; trnMnu(scrl, nil, nil); minFrm.Position, minFrm.Visible = ud2(0.5,-30,0,-50), true; tw(minFrm, tBnc, {Position=ud2(0.5,-30,0,10)}, true); bMax.Active = true end
    ...
    function UI_API.showConfirm() if isAnimating then return end; bMin.Visible, bCls.Visible = false, false; trnMnu(scrl, cnfFrm, ud2(0,150,0,80)); cnfEx(0) end
    ...
    function UI_API.showInput(ph, tx, btx) if isAnimating then return end; bMin.Visible, bCls.Visible = false, false; trnMnu(scrl, inpFrm, ud2(0,160,0,75)); inBox.PlaceholderText, inBox.Text, bSrch.Text = ph, tx, btx; inEx(0) end
    ...
    function UI_API.setButtonState(b, txt, act) if txt then b.Text = txt end; stBAct(b, act) end
    function UI_API.updateSpeedometerText(txt) spdoLbl.Text = txt end
    function UI_API.toggleSpeedometerVisibility(v) stLbl.Visible = not v; spdoLbl.Visible = v; spdoLbl.Position = v and ud2(0,5,1,-22) or ud2(0,5,1,-34) end
    function UI_API.sendNotif(ti, tx, d) SG:SetCore("SendNotification", {Title=ti, Text=tx, Duration=d}) end
