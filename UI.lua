...
return function(plr, CFG)
    ...
    local bSwitch -- ADDED: Reference for the Rayfield switch button
    local btns, bOrigClr = {}, {}
    ...
    -- ADDED: Switch Button in Configs Menu
    local rSwitch = mk("Frame", cScrl, {Size=ud2(0.92,0,0,20), BackgroundTransparency=1})
    bSwitch = crStylB(rSwitch, ud2(1,0,1,0), ud2(0,0,0,0), "SWITCH TO RAYFIELD", c3(255, 165, 0))
    bSwitch.Name = "SwitchButton"
    ...
    -- MODIFIED: Updated to handle the new Switch Button in config transitions
        ...
        for _,r in ipairs(cScrl:GetChildren()) do 
            if r:IsA("Frame") then 
                ...
                if btn then
                    ...
                    local bBg = btn:FindFirstChild("Background")
                    if bBg then 
                        tw(bBg, tFast, {BackgroundTransparency = a})
                        local bSt = bBg:FindFirstChildOfClass("UIStroke")
