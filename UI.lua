...
return function(plr, CFG)
    ...
    local function handleSnap(f)
        ...
        snapDebounce[f] = true
        local step = 20 + 4 -- Height + Padding
        ...
        if math.abs(currentY - targetY) > 1 then
            local twS = TS:Create(f, snapInfo, {CanvasPosition = Vector2.new(0, targetY)})
            twS:Play()
            twS.Completed:Wait()
        end
    ...
    for _, k in ipairs(sk) do 
        ...
        local showTt=function() ttLbl.Text=k; ttFrm.AnchorPoint=Vector2.new(0.5,1); ttFrm.Position=ud2(0,tTabBg.AbsolutePosition.X+(tTabBg.AbsoluteSize.X/2),0,tTabBg.AbsolutePosition.Y-5); ttFrm.Visible=true end; l.MouseEnter:Connect(showTt); l.MouseLeave:Connect(function() ttFrm.Visible=false end)
        local bb=mk("Frame", r, {Size=ud2(0.5,-4,1,0), Position=ud2(0.5,2,0,0), BackgroundColor3=CFG.ACCENT_COLOR, BackgroundTransparency=1, BorderSizePixel=0}); mk("UICorner", bb, {CornerRadius=UDim.new(0,4)}); mk("UIStroke", bb, {Color=CFG.BORDER_COLOR, Thickness=1, Transparency=1}); local bx=mk("TextBox", bb, {Size=ud2(1,-4,1,0), Position=ud2(0,2,0,0), BackgroundTransparency=1, Text=toStr(CFG[k]), TextColor3=CFG.SECONDARY_TEXT_COLOR, Font=Enum.Font.Gotham, TextSize=7, TextTransparency=1, ClearTextOnFocus=false})
    ...
    local function unTrn(t) tw(t, tBncIn, {Size=ud2(0,0,0,0), Position=ud2(0.5,0,0.5,0)}, true); t.Visible=false; local cx=main.Position.X; main.Size, main.Position, main.Visible = ud2(0,0,0,22), ud2(cx.Scale,cx.Offset,main.Position.Y.Scale,main.Position.Y.Offset), true; tw(main, tSmth, {Size=ud2(0,120,0,22), Position=ud2(cx.Scale, cx.Offset-60, main.Position.Y.Scale, main.Position.Y.Offset)}, true); fdMnu(0, true); shwUi(true, 120, 124); setA(0) end
    ...
    -- Restoration of Full API Functions
    ...
    function UI_API.toggleConfigMenu(isOpen)
        ...
        else tgBtns(1); task.wait(0.15); tw(main, tSmth, {Size=ud2(0,120,0,22)}, true); scrl.Visible, cScrl.Visible = false, true; shwUi(true, 120, 124); tgCfg(0) end; setA(0)
        isAnimating = false
    end
    ...
    function UI_API.drawPlayerEsp(p, c, tp, espState, dist, clr, sp, isOffscreen, vpCtr, bx, by)
        ...
        if isOffscreen then
            ...
            local dir = off.Unit; local scale = math.min(bx / math.abs(dir.X), by / math.abs(dir.Y))
            local pos = vpCtr + (dir * scale); l.Position, l.Visible = ud2(0, pos.X, 0, pos.Y), true
